    ...
    def run(self):
        try:
            if not self.dm.vision_available:
                self.error.emit("Vision data (STA/Params) is required for these metrics.")
                return
            ...
            for i, cid in enumerate(all_ids):
                ...
                # We need raw floats, not the formatted strings from analysis_core
                ...
                biphasic_idx = abs(secondary_val / (trough_val if is_off else peak_val))
                ...
                ellipticity = (stafit.std_y / stafit.std_x) if (stafit and stafit.std_x > 0) else 0
            ...
            if len(features) < 5:
                self.error.emit("Not enough valid clusters for UMAP (need > 5).")
                return
...
class UMAPPanel(QWidget):
    def __init__(self, main_window):
        ...
        self.run_btn.setStyleSheet("background-color: #4282DA; font-weight: bold;")
        
        self.color_combo = QComboBox()
        self.color_combo.addItems(["KSLabel", "Firing Rate", "ISI Violations", "Time to Peak (proxy)"])
    ...
    def on_finished(self, embedding, ids, metadata):
        ...
        self.main_window.status_bar.showMessage("UMAP Complete. Use Lasso (Mouse Drag) to select groups.")

    def update_plot(self, color_mode=None):
        if self.embedding is None: return
        ...
        self.ax.set_title(f"UMAP Projection (n={len(self.cluster_ids)})", color='white')
    ...
    def on_select(self, verts):
        ...
        if len(selected_ids) > 0:
            ...
            if reply == QMessageBox.Yes:
                from gui.callbacks import feature_extraction
    ...
    def create_group(self, ids):
        ...
        name, ok = QInputDialog.getText(self, "Group Name", "Enter name for this cluster group:")
        if ok and name:
