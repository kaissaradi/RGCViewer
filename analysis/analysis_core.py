...
# In analysis_core.py, replace the compute_sta_metrics function with this corrected version:

def compute_sta_metrics(sta_data, stafit, vision_params, cell_id):
    """
    Computes scalar metrics from the STA with enhanced metrics for RGC classification.
    ...
    if tc_matrix is not None:
        # Check if this is a black/white recording
        if tc_matrix.shape[1] == 1:
            # B/W recording
            dom_idx = 2  # Blue channel for B/W
            dom_trace = tc_matrix[:, 0]
            # Create dummy red/green traces for consistency
        ...
        if abs_max > 0:
            ...
            # Normalize all traces for color comparison
        ...
        # Use first 25% of trace as baseline (before stimulus onset)
        baseline_len = int(0.25 * len(smoothed_trace))
        if baseline_len > 5:
            baseline_std = np.std(smoothed_trace[:baseline_len])
            # Response is everything after baseline
        ...
        # Use original trace for zero crossing detection
