    ...
    *progress* is an optional ``callable(message, percent)``. Safe to call from
    ...
    if scalars is not None and len(scalars):
        for col, name, group, desc in (
            ...
            ("rf_area", "RF area", "Receptive field",
             "Area of the Vision Gaussian fit, π·σx·σy (stixels²)"),
            ("rf_long_diameter", "RF diameter (long)", "Receptive field",
             "2σ along the LONGER axis of the fitted RF ellipse (stixels)"),
            ("rf_short_diameter", "RF diameter (short)", "Receptive field",
             "2σ along the SHORTER axis of the fitted RF ellipse (stixels); "
             "long ≈ short for a round RF"),
            ("ellipticity", "RF ellipticity", "Receptive field",
             "σy/σx of the fit; 1 = round"),
