...
class WaveformPanel(QWidget):
    ...
    def update_all(self, cluster_id):
        ...
        features = self.main_window.data_manager.get_lightweight_features(
            cluster_id)

        if not features or 'raw_snippets' not in features:
            self.plot_widget.setTitle(
                f"Cluster {cluster_id} - Waiting for data...")
            return

        # Shape: (n_channels, n_time, n_spikes)
        snippets = features['raw_snippets']
        ...
        t_ms = (np.arange(n_samples) - 20) / \
            sr * 1000  # Center roughly on peak

        # 4. The Performance Trick (Connect with NaN)
        # We flatten all snippets into one single line separated by NaNs
        # This allows PyQtGraph to draw 100 spikes in 1 draw call instead of
        # 100.
        ...
        self.plot_widget.plot(
            t_ms, mean_trace, pen=pg.mkPen(
                color='#00e6a0', width=3))

        self.plot_widget.setTitle(
            f"Cluster {cluster_id} - Channel {dom_chan} ({n_spikes} snippets)")
