...
class GratingPanel(QWidget):
    ...
    def __init__(self, main_window):
        ...
        self._data_layout = data_layout   # kept so _clear_psth_grid can swap the widget in place
        ...
        self._psth_grid_positions = set()   # {(row, col)} occupied — informational only;
                                             # _clear_psth_grid swaps the whole widget rather
                                             # than removing items by position (see that method)
    ...
    def _render_cluster_data(self, cluster_id, data):
        ...
        if selection['condition'] is not None:
            ...
            display_cond = max(dsos_conditions,
                                key=lambda c: _abs_or_neg1(data[c].get('DSI', np.nan)))
            classification = 'none'
    ...
    def _draw_direction_psth_grid(self, entry, pref_dir_deg):
        ...
        closest_idx = None
        if directions_deg.size and not (isinstance(pref_dir_deg, float) and math.isnan(pref_dir_deg)):
            closest_idx = int(np.argmin(np.abs(((directions_deg - pref_dir_deg + 180) % 360) - 180)))
        ...
        compass_cells = {
            0: (1, 2), 45: (0, 2), 90: (0, 1), 135: (0, 0),
            180: (1, 0), 225: (2, 0), 270: (2, 1), 315: (2, 2),
        }
        ...
        for i in order:
            ...
            pen_color = colors.get('plot_compare', '#E03131') if is_pref else colors.get('plot_fr', '#FFD43B')
            plot_item.plot(t, rate, pen=pg.mkPen(pen_color, width=2 if is_pref else 1.5))

            label_color = colors['text_primary'] if is_pref else colors['text_tertiary']
            title_html = f"<span style='color:{label_color}; font-size:8px;'>{d:g}°</span>"
            plot_item.setTitle(title_html)

            self._psth_grid_plots[float(d)] = plot_item
            self._psth_grid_positions.add((row, col))