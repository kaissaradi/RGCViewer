...
_CONDITION_COLORS = ['#4DABF7', '#FF6B6B', '#69DB7C', '#FFD43B', '#B197FC', '#FF922B', '#66D9E8']
...
class GratingPanel(QWidget):
    ...
    def __init__(self, main_window):
        ...
        self._hist_bar_item = None         # BarGraphItem for the best condition, rebuilt per cluster
        self._hist_shadow_curves = []      # PlotCurveItems for non-best conditions, rebuilt per cluster
        plots_row.addWidget(self.hist_plot, stretch=1)

        data_layout.addLayout(plots_row, stretch=3)

        # Sanity-check strip: firing rate trace at the direction closest to
        # the best condition's preferred direction. Same visual language as
        # ChirpPanel's PSTH curve, deliberately not a raster.
        fr_label = QLabel(
            f"<span style='color:{colors['text_tertiary']}; font-size:9px;'>"
            f"FIRING RATE @ PREFERRED DIRECTION (sanity check)</span>"
        )
        data_layout.addWidget(fr_label)
        ...
        self._fr_curve = self.fr_plot.plot([], [], pen=pg.mkPen(colors.get('plot_fr', 'y'), width=2))
        data_layout.addWidget(self.fr_plot, stretch=1)
        ...
        self._sf_curve = self.sf_plot.plot([], [], pen=pg.mkPen(colors.get('plot_overlay', 'c'), width=2),
    ...
    def restyle_plots(self, colors):
        ...
        self._style_plot(self.fr_plot, colors)
        self._style_plot(self.sf_plot, colors)
        self._polar_pref_line.setPen(pg.mkPen(colors.get('plot_compare', 'r'), width=2, style=Qt.DashLine))
        self._fr_curve.setPen(pg.mkPen(colors.get('plot_fr', 'y'), width=2))
    ...
    def update_all(self, cluster_id):
        ...
        if not getattr(dm, 'grating_available', False) or dm.grating_status == 'missing':
    ...
    def _ensure_computing(self, cluster_id):
        ...
        thread.finished.connect(lambda cid=cluster_id: self._pending_workers.pop(cid, None))
    ...
    def cleanup(self):
        ...
        for plot in (self.polar_plot, self.hist_plot, self.fr_plot, self.sf_plot):
            plot.clear()
    ...
    def _render_cluster_data(self, cluster_id, data):
        ...
        dsos_conditions = sorted(
            (k for k in data if isinstance(k, tuple) and data[k].get('condition_type') == 'dsos'),
        )
        ...
        if not dsos_conditions:
            self.stats_label.setText("No direction-tuning conditions found for this cluster.")
        ...
        # "Best" = strongest |DSI| across conditions — same concept as the
        # reference notebook's best_sf/best_tf, just derived directly here.
        best_cond = max(
            dsos_conditions,
            key=lambda c: abs(data[c].get('DSI', 0.0)) if not math.isnan(data[c].get('DSI', float('nan'))) else -1.0
        )

        self._render_legend(dsos_conditions, best_cond, data)
        self._render_dsos_overlay(dsos_conditions, best_cond, data)
        self._render_hist(dsos_conditions, best_cond, data)

        best_entry = data[best_cond]
        dsi = best_entry.get('DSI', np.nan)
        osi = best_entry.get('OSI', np.nan)
        pref_dir = best_entry.get('preferred_direction_deg', np.nan)
        dsi_p = best_entry.get('DSI_pvalue', np.nan)
        osi_p = best_entry.get('OSI_pvalue', np.nan)
        bw, tf = best_cond
        self.stats_label.setText(
            f"Best: bw={bw:g} tf={tf:g}Hz   "
            f"DSI: {self._fmt(dsi)} (p={self._fmt(dsi_p, 3)})   "
            f"OSI: {self._fmt(osi)} (p={self._fmt(osi_p, 3)})   "
            f"Pref. dir: {self._fmt(pref_dir, 0)}°"
        )
        self._draw_firing_rate_sanity(best_entry, pref_dir)
    ...
    def _render_dsos_overlay(self, conditions, best_cond, data):
        ...
        for frac in (0.5, 1.0):
            ...
            ring = pg.PlotCurveItem(
                (r_max * frac) * np.cos(theta), (r_max * frac) * np.sin(theta), pen=ring_pen)
    ...
    def _render_hist(self, conditions, best_cond, data):
        ...
        best_color = _CONDITION_COLORS[conditions.index(best_cond) % len(_CONDITION_COLORS)]
        bar_brush = pg.mkColor(best_color)
        bar_brush.setAlpha(_BEST_ALPHA)

        self._hist_bar_item = pg.BarGraphItem(
            x=best_dirs_sorted, height=best_resp_sorted, width=bar_width,
        ...
        for i, cond in enumerate(conditions):
            ...
            curve = pg.PlotCurveItem(dirs[o], np.clip(resp[o], 0, None),
    ...
    def _draw_firing_rate_sanity(self, entry, pref_dir_deg):
        """
        Draws the mean firing-rate PSTH (Hz vs time, from grating_calc's
        cheap per-direction histogram — see psth_by_direction) at the
        direction closest to preferred_direction_deg, for the best
        condition. Answers "is there a real, time-locked response here, or
        is DSI picking out noise on a near-silent cell" — the thing the
        polar/histogram summaries can't show on their own.
        """
        self._polar_pref_line.setData([], [])
        directions_deg = np.asarray(entry['directions_deg'], dtype=float)

        if directions_deg.size == 0 or (isinstance(pref_dir_deg, float) and math.isnan(pref_dir_deg)):
            self._fr_curve.setData([], [])
            return

        closest_idx = int(np.argmin(np.abs(
            ((directions_deg - pref_dir_deg + 180) % 360) - 180)))
        closest_dir = directions_deg[closest_idx]

        # Overlay the preferred-direction spoke on the polar plot.
        mean_resp = np.asarray(entry['mean_response'], dtype=float)
        r_max = np.nanmax(mean_resp) if mean_resp.size else 1.0
        r_max = r_max if r_max > 0 else 1.0
        theta = math.radians(pref_dir_deg)
        self._polar_pref_line.setData([0, r_max * math.cos(theta)], [0, r_max * math.sin(theta)])

        psth_by_dir = entry.get('psth_by_direction')
        t_s = entry.get('psth_time_s')
        if psth_by_dir and t_s is not None and closest_dir in psth_by_dir:
            self._fr_curve.setData(np.asarray(t_s), np.asarray(psth_by_dir[closest_dir]))
            self.fr_plot.enableAutoRange()
        else:
            # Pre-analyzed files (loaded from disk rather than computed
            # on-demand) may not carry per-bin PSTHs — fall back to a flat
            # reference line at the scalar mean_response for that direction
            # rather than showing nothing.
            val = float(mean_resp[closest_idx])
            self._fr_curve.setData([0.0, 1.0], [val, val])
            self.fr_plot.enableAutoRange()