...
The project uses a dedicated Conda environment named `rgcviewer`. It is critical to maintain consistency by using this environment for all development and testing tasks.
...
```bash
# Run the app
conda run -n rgcviewer python run.py

# Run all tests
conda run -n rgcviewer pytest

# Install a new dependency
conda run -n rgcviewer pip install [package]
```
...
The asterisk `*` should be next to `rgcviewer`.
