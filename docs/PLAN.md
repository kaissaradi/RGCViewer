...
| Q14 | Feature Extraction default scatters | User | open | Wanted: Temporal PC1 v PC2, ACG PC1 v PC2, Temporal PC1 v ACG PC1, Temporal PC1 v RF area, then random. `_PLOT_META` in `feature_extraction.py`. User 2026-09-24: use RF **area** (not diameter). |
...
| Q32 | Vision `.sta`/`.params` made from a different sort than the loaded Kilosort run | Found 2026-09-24 | detection done; decision open | Law 1 pairing is only right if the Vision files came from this sort. `src/analysis/vision_sort_check.py` fits RF centre (x0, y0) against array position (`x_um`, `y_um`) over paired cells; robust R² < 0.15 = mismatch. Matched folders 0.38–0.86; ids shuffled < 0.01; **20251212A/kilosort40/data018: 0.03** (its `.sta` is dated 2025-12-18, before the 2026-01-27 sort; `.neurons` matches the sort 208/208). So data018's STAs, RF fits and Vision classes are on other cells. Shown as a permanent status-bar label (no modal; see `_on_vision_loaded` note) and in the Ctrl+S dialog. Open: hide or mark STA/RF for such runs? Regenerate data018's Vision files? Pick a matched default dataset for the harness. |
| Q33 | DSI/OSI significance: no multiple-comparison correction across conditions | Audit, Q9 | open (recommendation made) | Each (bw, TF) condition gets its own shuffle p for DSI and for OSI at 0.05; Auto picks a significant condition, so a cell has several chances. Recommendation: max-statistic permutation test across conditions (per shuffle, max DSI over conditions; p = rank of the observed max), one for DSI and one for OSI, keep the slider as the effect-size floor. Waiting for the user. |
