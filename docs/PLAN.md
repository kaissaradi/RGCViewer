...
| Q18 | Feature-extraction scatters have more contrast than the population view | Tester | needs detail | Feature Extraction points use the chrome accent `#1B4E9B` at alpha 0.65 (`feature_palette`), UMAP points `text_secondary` at 0.8. In light mode that makes FE points much stronger (≈8.5:1 vs ≈5.4:1 on white); in dark mode the order reverses. Not changed: ask tester which view she compares with (UMAP, RF mosaic, trace stacks) and in which theme, or for a screenshot. |
...
| Q20 | RF y-axis flip vs stimulus | Tester | checked; no flip on dev-testing | Vision stores fits y-up (`ImageFrame.fit`: y0 = height − row − 1 + 0.5) and draws the STA row 0 at the top. Encore: STA image `invertY` (row 0 top), ellipse via `rf_geometry.image_ellipse` (row = H − y0), mosaic plots y0 upward. Real data, peak-pixel row vs stored y0: y0 tracks H − row − 0.5 (corr 0.80 / 0.99 / 0.45; median error 1.8 / 0.5 / 0.6 stixels on 20260220A/data022, 20251212A/data018, 20260529A/data017); row + 0.5 misses by 11–24. Open: ask tester which build he saw (before `5903177`?). The data cannot show the physical monitor orientation, only that Encore matches Vision. |
