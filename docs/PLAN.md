...
| Q12 | `retinanalysis` ignored even when installed | Tester | done (needs decision) | Cause: `install.sh` makes an isolated venv (`include-system-site-packages = false`), so a conda or editable `retinanalysis` is invisible. Also: `load_stim_timing` sets `block_id` / `d_timing` and nothing reads them, so the package has no effect today. Warning lowered to info; README "Optional packages" shows how to install it into `~/.encore/.venv`. Open: does the user want Encore to use DataJoint epoch timing? |
